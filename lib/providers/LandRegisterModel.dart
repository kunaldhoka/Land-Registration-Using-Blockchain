...
class LandRegisterModel extends ChangeNotifier {
  ...
  Future<void> getAbi() async {
    ...
    _contractAddress = EthereumAddress.fromHex(jsonAbi["networks"]["5777"]["address"]);
        ...
        //     contractAddress); 
            //EthereumAddress.fromHex("0xD6af79CcaaCc6e1d747909d7580630aFc69Ff0B8"); //
  ...
  makePayment(dynamic reqId, dynamic price) async {
    notifyListeners();
    print("price = $price");
    print("price = ${(price * pow(10, 18)).toString()}");
    String priceS = (price * pow(10, 18)).toString();
    BigInt k=BigInt.zero;
    if(priceS.contains(".")){
      priceS = priceS.substring(0, priceS.indexOf("."));
      k = BigInt.parse(priceS)+BigInt.one;
    }
    else {k = BigInt.parse(priceS);}
    await _client.sendTransaction(
        _credentials,
        Transaction.callContract(
            contract: _contract,
            ...
            value: EtherAmount.fromUnitAndValue(
                EtherUnit.wei,
                k 
                // BigInt.parse((price * pow(10, 18)).toString())
                )
                ),
        chainId: _chainId,
        fetchChainIdFromNetworkId: false);
